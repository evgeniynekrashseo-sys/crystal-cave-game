import {CITY_RESEARCH} from './city-research.js?v=20';
import {mountCityHud} from './city-hud.js?v=20';
import {drawSettlement,mapTile} from './settlement-renderer.js?v=20';
import {bindMapControls,boundCamera,projectPoint,MIN_ZOOM,MAX_ZOOM} from './settlement-camera.js?v=20';
import {SIZE,TYPES,TECH,LABELS,deliverDiscovery,terrain,normalizeCity,onChemistryWin,stats,step,build,canPlace,upgrade,upgradeCost,expand,research,affordable,demolish,demolitionRefund,VOYAGES,voyageError,dispatchVoyage,claimCityCoins,cityGoal,claimCityGoal} from './settlement-model.js?v=20';
...
export function initCity(api){
 ...
 function hud(){
  ...
  mission.querySelector('span').textContent=goal?(goal.test(c)?`Готово: +${goal.coins} монет`:goal.hint):'Освіта, торгівля та нові відкриття';
 ...
 if(tab==='build'&&b&&!tool){
  target.insertAdjacentHTML('beforeend',`<article class="city-selected-summary"><b>${TYPES[b.type].name} · рівень ${b.level}/5</b><p>${TYPES[b.type].desc}</p><small>Працівників: ${c.agents.filter(a=>a.job===b.id).length}${b.type==='school'?` · Освіта ${stats(c).education}% · Учнів ${stats(c).students}`:''}</small></article>${b.type==='harbor'?`<article><b>Морські контракти</b><p data-voyage-time></p>${Object.entries(VOYAGES).map(([id,v])=>`<div class="city-contract"><b>${v.name}</b><small>Вантаж: ${costText(v.cost)} · ${v.duration} с</small><small>Повернення: ${v.coins*b.level} монет · ${costText(Object.fromEntries(Object.entries(v.reward).map(([k,n])=>[k,n*b.level])))}</small><button data-voyage="${id}" ${voyageError(c,b,id)?'disabled':''}>Відправити корабель</button></div>`).join('')}</article>`:''}`);
 }
 ...
 for(const [id,dx,dy] of [['left',-1,0],['right',1,0],['up',0,-1],['down',0,1]])bind('tile-'+id,()=>{selected={x:Math.max(0,Math.min(c.extent-1,selected.x+dx)),y:Math.max(0,Math.min(c.extent-1,selected.y+dy))};pendingDemolition=false;panel()});
