...
const diag:ChemLabDiagnostics={build:'V58',startedAt:performance.now(),longTasks:0,errors:[]};
...
window.addEventListener('load',()=>{
  ...
  for(const p of performance.getEntriesByType('paint')){if(p.name==='first-paint')diag.firstPaintMs=Math.round(p.startTime);if(p.name==='first-contentful-paint')diag.firstContentfulPaintMs=Math.round(p.startTime)}
});
...
function sample(now:number){const dt=now-last;last=now;frames++;total+=dt;worst=Math.max(worst,dt);if(now-start<5000){requestAnimationFrame(sample);return}const elapsed=now-start;diag.frameSample={fps:Math.round((frames/elapsed)*1000),avgFrameMs:+(total/frames).toFixed(2),worstFrameMs:+worst.toFixed(2),frames};console.info('[ChemLab Diagnostics]',diag)}
