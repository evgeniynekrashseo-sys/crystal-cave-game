...
function geometry(n:number){const mobile=boardW<=520,two=(mobile&&n>=5)||n>=7,cols=mobile&&n>=5?3:(n>=7?4:n),desired=mobile&&n>=5?96:n<=4?100:n===5?92:n===6?84:78,gap=mobile&&n>=5?Math.max(14,Math.min(24,(boardW-cols*desired)/(cols+1))):Math.max(16,Math.min(32,(boardW-cols*desired)/(cols+1))),w=Math.max(64,Math.min(desired,(boardW-gap*(cols+1))/cols)),h=w*3.35,total=cols*w+(cols-1)*gap;return{two,cols,gap,w,h,start:(boardW-total)/2}}
function resize(){if(!app)return;boardW=Math.max(280,Math.floor(els.board.getBoundingClientRect().width));const g=geometry(tubes.length),rows=Math.max(1,Math.ceil(tubes.length/g.cols));boardH=rows>1?Math.ceil(38+rows*g.h+(rows-1)*46+28):Math.max(390,Math.round(boardW*.55));app.renderer.resize(boardW,boardH);renderBoard()}
function drawTube(t:Tube,i:number,g:ReturnType<typeof geometry>){const row=g.two?Math.floor(i/g.cols):0,col=i%g.cols,rowCount=g.two?Math.min(g.cols,tubes.length-row*g.cols):g.cols,rowWidth=rowCount*g.w+(rowCount-1)*g.gap,rowStart=(boardW-rowWidth)/2,holder=new Container();holder.x=rowStart+col*(g.w+g.gap);holder.y=28+row*(g.h+46);holder.eventMode='static';holder.cursor='pointer';holder.on('pointertap',()=>void tap(i));drawPremiumTube(holder,t,s=>COLORS[s as SymbolKey],g,selected===i);if(selected===i)holder.y-=6;root.addChild(holder);tubeViews[i]=holder}
