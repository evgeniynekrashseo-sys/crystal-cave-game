...
function drawTube(t:Tube,i:number,g:ReturnType<typeof geometry>){const row=g.two?Math.floor(i/g.cols):0,col=i%g.cols,rowCount=g.two?Math.min(g.cols,tubes.length-row*g.cols):g.cols,rowWidth=rowCount*g.w+(rowCount-1)*g.gap,rowStart=(boardW-rowWidth)/2,holder=new Container();holder.x=rowStart+col*(g.w+g.gap);holder.y=24+row*(g.h+38);holder.eventMode='static';holder.cursor='pointer';holder.on('pointertap',()=>void tap(i));drawPremiumTube(holder,t,s=>COLORS[s as SymbolKey],g,selected===i);if(P.nug>=3){const aura=new Graphics().roundRect(2,5,g.w-4,g.h-5,g.w*.34).stroke({color:P.nug>=10?0xffc83d:P.nug>=6?0xa95cff:0x39ddff,width:P.nug>=10?1.8:1.2,alpha:.22});holder.addChildAt(aura,0)}if(selected===i)holder.y-=5;root.addChild(holder);tubeViews[i]=holder}
...
function win(){const reward=50+P.level*5+Math.min(30,moves*2),xp=25+P.level*2,clean=assistsUsed===0,mastery=clean?1:0,before=P.nug;P.gold+=reward;P.xp+=xp;P.nug+=mastery;P.streak=(P.streak||0)+1;P.bestStreak=Math.max(P.bestStreak||0,P.streak);save();const unlock=mastery&&before<3&&P.nug>=3?' · відкрито Neon Glass':mastery&&before<6&&P.nug>=6?' · відкрито Violet Glass':mastery&&before<10&&P.nug>=10?' · відкрито Gold Glass':'';els.winReward.textContent=`+${reward} кредитів · +${xp} XP${mastery?' · +1 кристал майстерності':''} · серія ${P.streak}${unlock}`;render();tone(clean?1180:1040,.2,.04);haptic(clean?35:25);els.win.classList.add('show')}
...
document.getElementById('hub')?.addEventListener('click',()=>{const glass=P.nug>=10?'Gold Glass':P.nug>=6?'Violet Glass':P.nug>=3?'Neon Glass':'Base Glass';toast(`Лабораторія · ${P.discovered.length}/118 · серія ${P.bestStreak||0} · ${glass} · кристали ${P.nug}`)});
