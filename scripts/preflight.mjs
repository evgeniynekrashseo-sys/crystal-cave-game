...
const cosmeticPreview=read('src/v102-cosmetic-equip-preview.ts');

const build=`V${String(pkg.version).split('.')[0]}`;

if(pkg.version!=='102.0.0') fail(`package version must be 102.0.0, got ${pkg.version}`); else pass(`release version ${pkg.version}`);
...
if(cosmeticPreview.includes("localStorage.setItem('chemlab_v50'")||cosmeticPreview.includes('localStorage.setItem("chemlab_v50"')) fail('cosmetic preview must not write core save'); else pass('cosmetic preview is isolated from core save');
...
const sourceBundle=[main,certified,cosmeticPreview].join('\n');
if(forbiddenOperationalPatterns.some(r=>r.test(sourceBundle))) fail('potentially operational real-world chemistry instruction detected in core sources'); else pass('no operational chemistry quantities/instructions detected in release sources');
